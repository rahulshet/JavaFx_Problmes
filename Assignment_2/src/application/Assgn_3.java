...
public class Assgn_3 extends Application {
	Label response;
	...
	public void start(Stage myStage) {
		...
		response = new Label("Context Menu");
		
		Menu New = new Menu("New");
		MenuItem file = new MenuItem("File");
		MenuItem folder  = new MenuItem("Folder");
		MenuItem image  = new MenuItem("Image");
		New.getItems().addAll(file, folder, image, new SeparatorMenuItem());
		Menu view = new Menu("View");
		MenuItem large  = new MenuItem("large");
		MenuItem medium  = new MenuItem("medium");
		MenuItem small  = new MenuItem("samll");
		view.getItems().addAll(large, medium, small, new SeparatorMenuItem());
		// Create a context (i.e., popup) menu that shows edit options.
		final ContextMenu editMenu = new ContextMenu(New, view);
